  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://stock-trading-platform-backend-3svr.onrender.com/login",
        { ...inputValue },
      ...
      if (success) {
        ...
        setTimeout(() => {
          window.location.href = `https://stock-trading-platform-nfb3.vercel.app?token=${data.token}`; // dashboard port 
        }, 1000);
      } else {
        handleError(message);
      }
